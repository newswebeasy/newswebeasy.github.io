...
$(document).ready(function () {
  ...
  $.getJSON(url_calendar, function (resp) {
    let dt = new Date(parseInt(page.yyyy), parseInt(page.mm) - 1);
    ...
    while (di <= max_date) {
      ...
      if (di.getMonth() == dt.getMonth()) {
        ...
        if (has_news) {
          ...
          let url = `/news/${page.categories}/${page.yyyy}/${page.mm}/${dd}/`;
    ...
    dates = dates.filter(x => x[1][page.categories] !== undefined);
    ...
    let prevDayUrl;
    if (dd_index > 0) {
      prevDayUrl = `/news/${page.categories}/${page.yyyy}/${page.mm}/${dates[dd_index - 1]}/`;
    } else if (dd_index === 0) {
    ...
    if (dd_index < dates.length - 1) {
      nextDayUrl = `/news/${page.categories}/${page.yyyy}/${page.mm}/${dates[dd_index + 1]}/`;
